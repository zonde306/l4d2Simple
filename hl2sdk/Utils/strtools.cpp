...
void V_AppendSlash(char *pStr, int strSize)
{
	...
	{
		if (len + 1 >= strSize)
			AssertMsg("V_AppendSlash: ran out of space on %s.", pStr);
...
void V_MakeAbsolutePath(char *pOut, int outLen, const char *pPath, const char *pStartingDir)
{
	...
	else
	{
		...
		else
		{
			if (!_getcwd(pOut, outLen))
				AssertMsg(true, "V_MakeAbsolutePath: _getcwd failed.");
	...
	if (!V_RemoveDotSlashes(pOut))
		AssertMsg(true, "V_MakeAbsolutePath: tried to \"..\" past the root.");
